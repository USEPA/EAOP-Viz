...
#### DESCRIPTION OF CODE
## This code was created 12/17/2018 to support the analyses of the manuscript entitled
## "Benchmarking and Extracting Emerging Adverse Outcome Pathway Knowledge"
## by N.Pollesch, D. Villenueve, and J. O'Brien
##   This code will also serve as the basis for the R package, 'AOPNet' that
##   is currently under development by these authors.
...
###################################################################
## FUNCTION: date.time
## Return a simplified date_time string for file naming
###################################################################
#
### Description: Creates a string for file naming with hours, minutes, and seconds (optional)
#
### Input: sec Include seconds? Default=FALSE [boolean]
#
### Output: returns a string with hours, minutes, and seconds (optional) [string]

date.time<-function(sec=F){
  d<-substring(Sys.time(),6,10)
  h<-substring(Sys.time(),12,13)
  m<-substring(Sys.time(),15,16)
  s<-substring(Sys.time(),18,19)
  if(sec){
    return(paste(d,"_",h,"-",m,"-",s,sep=""))}
  else{
    return(paste(d,"_",h,"-",m,sep=""))
  }
}
